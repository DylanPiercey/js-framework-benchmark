...
detect_chrome() {
  ...
  # headless startup in some sandboxes. Cover both the /opt/pw-browsers layout
  # and Playwright's default ~/.cache/ms-playwright cache, and both the newer
  # `chrome-headless-shell` and older `headless_shell` binary names.
  for d in \
