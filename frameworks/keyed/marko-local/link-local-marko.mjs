...
function run() {
  ...
  // Marko migrated to pnpm workspaces; use pnpm when its lockfile is present.
  const pnpm = fs.existsSync(path.join(markoRepo, "pnpm-lock.yaml"));
  if (!fs.existsSync(path.join(markoRepo, "node_modules"))) {
    exec(pnpm ? "pnpm install" : "npm install", markoRepo);
  }

  // Build the production output for the packages we vendor.
  exec(
    pnpm
      ? `pnpm ${packages.map((name) => `--filter @marko/${name}`).join(" ")} run build`
