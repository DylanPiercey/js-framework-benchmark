...
* **Browser selection.** The full `chrome` binary can hang on headless startup
  in some sandboxes, so the script prefers Playwright's `headless_shell`
  (`/opt/pw-browsers/chromium_headless_shell-*`) when present, then a prebuilt
  `chromium-*`, then a system `google-chrome`/`chromium`. Override with
